 ...
 */
public class ComputePrime2 {

    private static int WheelFactorization(long startCircles, long endCircles, int startPosition, int endPosition, int N, int[] cases){
        //parameters
        int sum = 0, flag;
        long x; //possible prime
        //computation
        for(int i : cases){ //possible numbers in each circle
            long numberOfCircles = startCircles;
            while(numberOfCircles <= endCircles){ //for each circle
                if(numberOfCircles == startCircles && i < startPosition){ //doesn't reach the first possible number
                    numberOfCircles++;
                    continue;
                }
